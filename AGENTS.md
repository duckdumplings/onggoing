...
| `dispatch` | `src/domains/dispatch/` | 미구현 | 배차 로직은 현재 `src/app/api/route-optimization/route.ts` 안에 인라인 |
...
| `evals/chatEvalCases.ts` | AI Chat 평가 케이스 |
...
| `quote/ai-chat-generate/route.ts` | AI 견적 챗봇 메인 핸들러 | **2661** | **매우 높음** |
...
| `src/app/api/quote/ai-chat-generate/route.ts` | 2661 | 일부 services로 이미 분리, 나머지도 이전 필요 |
...
- `NEXT_PUBLIC_APP_URL`
- LLM 키 (`ai-chat-generate` 사용) — `.env.local` 확인
