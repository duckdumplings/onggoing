  ...
  return {
    ...
    quote_case_board: tool({
      ...
      inputSchema: z.object({
        ...
        monthlyBasis: z
          .enum(['calendar', 'average'])
          .default('calendar')
          .describe('월 산정 기준. 특정 월 실제 달력이면 calendar, "월 평균 운영일수" 기준이면 average.'),
        targetMonth: z
          .string()
          .regex(/^\d{4}-\d{2}$/)
          .optional()
      ...
      execute: async ({ cases, contractMonths, monthlyBasis, targetMonth }) => {
        ...
        track(
          'quote_case_board',
          { count: cases.length, contractMonths: contractMonths ?? null, monthlyBasis, targetMonth: targetMonth ?? null },
