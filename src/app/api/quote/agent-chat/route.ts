...
- 사용자가 "경유지별 운행시간/체류시간", "구간별로 얼마나 걸렸나", "어느 구간도 지체 없었다는 근거" 등을 원하고 지점별 완료시각을 줬다면, audit_delivery_timeline의 stopTimeline에 방문순서대로(첫 항목=출발/상차, completedAt=출발시각) 넣어 구간별 분해표(legs: 이론 주행 vs 실측 간격 → 추정 체류)를 받아 마크다운 표로 제시하라. 이미 직전 턴에서 받은 주소·완료시각이 있으면 다시 묻지 말고 그대로 재사용하라.
...
function applyToolResult(acc: CollectedOutputs, toolName: string, output: any): void {
  ...
  } else if (toolName === 'audit_delivery_timeline' && output?.routeRequest) {
    // 사후 진단의 최적 경로도 지도에서 확인 가능하도록 routeRequest 노출.
    acc.routeRequest = output.routeRequest;
