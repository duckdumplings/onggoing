...
- 각 케이스(cases[])는: label(예 "강동&잠실&송파&하남 점심"), group(권역 묶음, 예 "권역1"), 역할 태깅된 stops(수거/배송/반납), vehicleType, departureTime("HH:mm"), deadline("HH:mm"), deadlineTarget(기본 delivery), planPreference, frequency, monthlyVisits(그 달 운행 횟수)로 구성한다.
...
- 월간/계약 합계가 필요하면 각 케이스 monthlyVisits와 contractMonths를 채워라. 합산·곱셈은 도구(rollup)가 하니 본문에서 직접 곱하지 마라. 보드 결과(cases, rollup)의 수치(1회/월/연/계약)만 그대로 옮겨라. "일 단위"를 물으면 그 날 운행하는 케이스(점심+저녁 등)의 1회 운임 합을 제시하라(이 역시 본문에서 새 숫자를 만들지 말고 케이스 oneTimePrice를 더한 값으로 설명).
