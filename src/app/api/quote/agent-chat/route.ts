...
- calculate_quote가 costReference(예상 유류비·예상 통행료)를 함께 돌려준다. 이것은 요금제 청구액과 별개인 "운영 참고 추정치"다. 사용자가 "유류비/주유비/기름값이 얼마나 드냐", "톨비/통행료는?" 등 실비를 물으면, costReference의 estimatedFuel(유가 ₩fuelPricePerLiter/L · 연비 fuelEfficiencyKmPerL km/L 기준 실주행 연료비 추정)과 estimatedToll(거리 기반 통행료 추정)을 그 값 그대로 안내하라. 유가 출처(fuelPriceSourceLabel — 오피넷 전국 평균/수동 설정/기본 가정)도 한 줄 밝혀라. 단, 이 수치는 직접 계산하지 말고 costReference 값만 쓰고, "참고 추정치이며 유가·경로·실제 통행료 구간에 따라 달라질 수 있다"고 반드시 덧붙여라. 예상 유류비(실주행 연료비)와 유류할증(시간당 요금제 청구 항목)은 다른 개념이니 혼동하지 마라.
  ...
  try {
    ...
    const stream = new ReadableStream<Uint8Array>({
      async start(controller) {
        ...
        const hasAnyOutput = () =>
          Boolean(acc.agentQuote || acc.scenarioComparison || acc.routeRequest || acc.departureMatrix || acc.askedQuestion);
        ...
        const hasQuoteOutput = Boolean(acc.agentQuote) || Boolean(acc.scenarioComparison);
